...
local clickButton = function(Cat)
    for _,v in pairs(Cat) do
        RageUI.List(v.Label.." ~g~"..v.Price.."$~c~/u~s~", indexList.myList, indexList.Index1, nil, {}, true, {
            ...
            onSelected = function(i, Item)
                ...
                elseif id == 4 then
                    local myAmount = KeyboardInput("Combien?", '', 2)
                    if myAmount ~= nil and myAmount ~= " " then
                        if myAmount then
                            ...
                            Notif("~r~Erreur: Syntaxe incorrecte")
                            return
                        end
                    else
                        Notif("~r~Erreur: Syntaxe incorrecte")
...
local drawPaiement = function()
    RageUI.Separator("~y~Mode paiement:~s~ ~g~"..tostring(ChoixPaiement))
end

function openShopMenu()
    local OpenedMenu = false
    local mainMenu = RageUI.CreateMenu("Superette", 120, 200)
    local nourritureMenu = RageUI.CreateSubMenu(mainMenu, "Nourriture", 120, 200)
    local boissonsMenu = RageUI.CreateSubMenu(mainMenu, "Boissons", 120, 200)
    local divsMenu = RageUI.CreateSubMenu(mainMenu, "Divers", 120, 200)
